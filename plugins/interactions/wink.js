...
module.exports = {
    ...
    async execute(sock, m, args, config) {
        try {
            ...
            if (!target) return sock.sendMessage(m.chat, { text: "❌ Mention or reply to someone.\nExample: `.wink @user`" }, { quoted: m });

            const { buffer, mimetype } = await fetchGif('wink');
            await sock.sendMessage(m.chat, {
                video: buffer, mimetype,
                gifPlayback: true,
                caption: `😉 *${m.pushName || "Someone"}* winked at *@${target.split("@")[0]}*`,
                mentions: [target]
            }, { quoted: m });
        } catch (err) {
            console.error(err);
            await sock.sendMessage(m.chat, { text: "❌ Failed to wink." }, { quoted: m });
        }
    }
};