...
module.exports = {
    ...
    async execute(sock, m, args, config) {
        try {
            ...
            if (!target) return sock.sendMessage(m.chat, { text: "❌ Mention or reply to someone.\nExample: `.stare @user`" }, { quoted: m });

            const { buffer, mimetype } = await fetchGif('stare');
            await sock.sendMessage(m.chat, {
                video: buffer, mimetype,
                gifPlayback: true,
                caption: `👀 *${m.pushName || "Someone"}* is staring at *@${target.split("@")[0]}*`,
                mentions: [target]
            }, { quoted: m });
        } catch (err) {
            console.error(err);
            await sock.sendMessage(m.chat, { text: "❌ Failed to stare." }, { quoted: m });
        }
    }
};