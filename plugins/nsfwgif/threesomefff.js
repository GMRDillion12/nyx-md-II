...
module.exports = {
    command: ["fff"],
    category: "nsfwgif",
    description: "Get a random NSFW threesome (FFF) GIF",

    execute: async (sock, m) => {
        try {
            await sock.sendMessage(m.chat, { text: "🔞 Loading threesome (FFF) GIF..." });

            const response = await axios.get("https://api.purrbot.site/v2/img/nsfw/threesome_fff/gif", {
                timeout: 10000,
            });
            ...
            await sock.sendMessage(m.chat, {
                ...
                gifPlayback: true,
                mimetype: "video/mp4",
                caption: "🔞 Here's your random threesome (FFF) GIF! Enjoy the animation 💦😏",
            }, { quoted: m });
            ...
            console.error("ThreesomeFFF GIF command error:", err.message || err);

            let replyText = "❌ Failed to fetch threesome (FFF) GIF. Try again later! (No GIF available this time?)";
