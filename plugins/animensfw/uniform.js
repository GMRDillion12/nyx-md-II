...
module.exports = {
    command: ["nuniform"],
    category: "animensfw",
    description: "Get a random SFW uniform image",

    execute: async (sock, m) => {
        try {
            await sock.sendMessage(m.chat, { text: "🩷 Loading uniform image..." });

            const response = await axios.get("https://api.waifu.im/images", {
                params: {
                    includedTags: "uniform",
                    isNsfw: true,
                },
                timeout: 10000,
            });
            ...
            await sock.sendMessage(m.chat, {
                image: buffer,
                caption: "🩷 Here's your random uniform! Enjoy 💕",
            }, { quoted: m });
            ...
            if (err.response) {
                ...
                replyText = "❌ Request timed out. API might be slow.";                                                                          } else if (err.message.includes("No valid image")) {                                                                                     replyText = "❌ No image found this time. Try again!";
            }
